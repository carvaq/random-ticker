...
class BookmarksViewModelTest {
    ...
    private val tickerPreferences = spy(TickerPreferences(ApplicationProvider.getApplicationContext()))
    private val repositoryMock: BookmarkRepository = mock()

    private val testee = BookmarksViewModel(SavedStateHandle(emptyMap()), tickerPreferences, repositoryMock)

    @Test
    fun `when deleting a bookmark then delete in on the repository`() = coroutinesRule.runBlockingTest {
        //given
    ...
    @Test
    fun `when creating a new bookmark save it ticker preferences`() = coroutinesRule.runBlockingTest {
        //given
